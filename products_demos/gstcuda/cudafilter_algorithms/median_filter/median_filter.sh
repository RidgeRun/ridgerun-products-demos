...
echo -e "\n === GstCUDA Median Filter ===\n"

camera_flag=false
CAMERA_ID=()
mp4_flag=false
display_flag=true
OUTVIDEO=median_filter_test.mp4
...
while getopts "m:w:h:d?help?" flag; do
   ...
   d)
	...
	;;
   *|help)
        echo "RidgeRun GstCUDA Median Filter"
	echo ""
...
GST_DEBUG=2 gst-launch-1.0 -e $INPUT ! nvvidconv ! "video/x-raw(memory:NVMM),width=$W,height=$H,format=I420" ! \
cudafilter in-place=true location=./median-filter.so ! $OUTPUT
