...
if [ "$mp4_flag" = true ]; then
	...
	INPUT="videotestsrc is-live=true pattern=ball ! video/x-raw,width="$W",height="$H",format=I420,framerate=60/1 "
fi
...
if [ "$display_flag" = false ];then
	...
	OUTPUT=" nvv4l2h264enc bitrate=20000000 ! h264parse ! mp4mux ! filesink location=$OUTVIDEO"
fi

GST_DEBUG=2 gst-launch-1.0 -e cudamux name=cuda in-place=true location=./mixer.so $INPUT ! nvvidconv ! "video/x-raw(memory:NVMM),width=$W,height=$H,format=I420" ! \
queue ! cuda.sink_0 $INPUT ! nvvidconv ! "video/x-raw(memory:NVMM),width=$W,height=$H,format=I420,framerate=60/1" ! queue ! cuda.sink_1 cuda. ! $OUTPUT
