...
/*
 * The Push Method selects the data type for the metadata:
 * 0: SIGNAL
 * 1: BINARY DATA
 * 2: STRING
 */
#define PUSH_META_METHOD 0
...
main (int argc, char *argv[])
{
  ...
  /* Default settings */
  length = 10;
  output = g_strdup ("./metadata_video.ts");
...
gst_meta_demo_parse_args (int *argc, char **argv[])
{
  ...
  {"output", 'o', 0, G_OPTION_ARG_FILENAME, &output,
      "The location of the output file (defaults to ./metadata_video.ts)", "F"},
  {NULL}
};
    ...
    gpointer user_data)
{
  ...
  meta = g_strdup_printf ("Buffer %04" G_GUINT64_FORMAT " - %" GST_TIME_FORMAT "\n", offset,
      GST_TIME_ARGS (ts));
  metalen = strlen (meta);
